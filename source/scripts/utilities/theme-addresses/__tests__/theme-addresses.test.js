...
describe('AddressForm', () => {
  beforeAll(() => {
    fetchMock.mock('https://country-service.shopifycloud.com/graphql', countries);
  ...
  describe('with no custom options', () => {
    ...
    test('use [data-default] country value if set in the DOM', () => {
      const provinceSelect = document.body.querySelector('[name="address[province]"]');
    ...
    test('reorders field correctly', () => {
      ...
      let inputs = [...document.body.querySelectorAll('[data-address=root] [name^=address]')];
      ...
      inputs = [...document.body.querySelectorAll('[data-address=root] [name^=address]')];
    ...
    test('replaces labels depending of the country', () => {
      const address2Label = document.body.querySelector('[for="AddressAddress2"]');
    ...
    test('provinces select is hidden if country does not have any', () => {
      const provinceWrapper = document.body.querySelector('[data-province-wrapper]');
      const provinceSelect = document.body.querySelector('[name="address[province]"]');
  ...
  describe('with custom options', () => {
    ...
    beforeEach((loaded) => {
      ...
      customCountrySelect = document.body.querySelector('#CustomAddressCountry');
    });
